...
def _seed_documents():
    """Ingest the synthetic resume into ChromaDB using the full production pipeline."""
    resume_path_str = str(RESUME_PATH)
    ingest_file_for_eval(resume_path_str, EVAL_CANDIDATE_ID, doc_type="cv")
    print(f"[Harness] Ingested synthetic resume via production path into '{EVAL_CANDIDATE_ID}'")
...
def _cleanup_eval_collections():
    ...
    try:
        ...
        print(f"[Harness] Cleaned up ChromaDB summary collection")
    except Exception:
        pass
...
def run_evaluation(
    ...
    if components is None:
        components = ["tool_selection", "rag", "geval", "refusal", "ingestion", "router"]
    ...
    try:
        ...
        if dry_run:
            ...
            # ── 4a. Tool Selection ──────────────────────────────────
            if "tool_selection" in components:
                print("\n" + "─" * 50)
                print("  Component: Tool Selection")
                print("─" * 50)
                from evaluation.evaluators.tool_evaluator import run_tool_evaluation
                eval_results["tool_eval_df"] = run_tool_evaluation(pipeline_results)
                eval_results["tool_eval_df"].to_csv(
                    REPORTS_DIR / "tool_selection_scores.csv", index=False
                )

            # ── 4b. RAG Quality (RAGAS + Hallucination) ─────────────
            if "rag" in components:
                print("\n" + "─" * 50)
                print("  Component: RAG Quality")
                print("─" * 50)
                # Filter to questions where search_documents was actually used
                rag_data = [
                ...
                if rag_data:
                    ...
                    eval_results["ragas_df"] = run_ragas_evaluation(rag_data, judge_model=judge_model)
                    eval_results["ragas_df"].to_csv(
                        REPORTS_DIR / "ragas_scores.csv", index=False
                    )

                    # DeepEval Hallucination
                    from evaluation.evaluators.deepeval_evaluator import run_deepeval_hallucination
                    eval_results["hallucination_df"] = run_deepeval_hallucination(
                        rag_data, judge_model=judge_model
                    )
                    eval_results["hallucination_df"].to_csv(
                        REPORTS_DIR / "hallucination_scores.csv", index=False
                    )
                else:
                    print("[Harness] No RAG-routed questions — skipping RAGAS and Hallucination")

            # ── 4c. Answer Correctness (GEval) ──────────────────────
            if "geval" in components:
                print("\n" + "─" * 50)
                print("  Component: Answer Correctness (GEval)")
                print("─" * 50)
                # Run on all questions except negative
                geval_data = [
                    r for r in pipeline_results
                ...
                if geval_data:
                    ...
                    eval_results["geval_df"] = run_deepeval_geval(
                        geval_data, judge_model=judge_model
                    )
                    eval_results["geval_df"].to_csv(
                        REPORTS_DIR / "geval_scores.csv", index=False
                    )

            # ── 4d. Refusal Accuracy ────────────────────────────────
            if "refusal" in components:
                print("\n" + "─" * 50)
                print("  Component: Refusal Accuracy")
                print("─" * 50)
                negative_data = [
                    r for r in pipeline_results
                    if r["category"] == "negative"
                ...
                if negative_data:
                    ...
                    eval_results["refusal_df"] = run_refusal_evaluation(negative_data)
                    eval_results["refusal_df"].to_csv(
                        REPORTS_DIR / "refusal_scores.csv", index=False
                    )

            # ── 4e. Ingestion Quality ───────────────────────────────
            if "ingestion" in components:
                print("\n" + "─" * 50)
                print("  Component: Ingestion Quality")
                print("─" * 50)
                from evaluation.evaluators.ingestion_evaluator import run_ingestion_evaluation
                eval_results["ingestion_report"] = run_ingestion_evaluation(
                    EVAL_CANDIDATE_ID, judge_model=judge_model
                )
                # Save as JSON
                ingestion_path = REPORTS_DIR / "ingestion_report.json"
                with open(ingestion_path, "w", encoding="utf-8") as f:
                    json.dump(eval_results["ingestion_report"], f, indent=2, ensure_ascii=False)

            # ── 4f. Router Accuracy ─────────────────────────────────
            if "router" in components:
                print("\n" + "─" * 50)
                print("  Component: Router Accuracy")
                print("─" * 50)
                # Filter to docs-expected questions with expected_route set
                router_data = [
                    r for r in pipeline_results
                    if r.get("expected_route") is not None
                ]
                ...
                if router_data:
                    ...
                    eval_results["router_df"] = run_router_evaluation(router_data)
                    eval_results["router_df"].to_csv(
                        REPORTS_DIR / "router_scores.csv", index=False
                    )
